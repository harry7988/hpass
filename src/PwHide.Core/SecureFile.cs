...
public static class SecureFile
{
    ...
    private static void InstallStagedAsRoot(string stagingPath, string finalPath)
    {
        ...
        var orig = finalPath + ".pwhide-orig-" + Guid.NewGuid().ToString("N");
        var fresh = finalPath + ".pwhide-new-" + Guid.NewGuid().ToString("N");
    ...
    {
        // fd-based 读取（root 场景的命门，全 Unix 平台）：open(O_NOFOLLOW) 后基于已锁定 inode
        // （Linux=/proc/self/fd/N、Darwin=/dev/fd/N）复核属主/大小，再从 fd 读入——窗口内把 staging
        // 换成指向 root 专属文件的链接也会被属主复核拒绝（跨特权读取闭环）
        var expectedOwner = -1;
        ...
        try
        {
            ...
            {
                "vault.json" => JsonSerializer.Deserialize(content, PwHideJsonContext.Default.VaultFile),
                "master.key" => JsonSerializer.Deserialize(content, PwHideJsonContext.Default.MasterKeyFile),
                "config.json" => JsonSerializer.Deserialize(content, PwHideJsonContext.Default.PwHideConfig),
    ...
    private static void RejectSymlinkedStagingAncestors(string stagingPath)
    {
        ...
        var home = runDir is null ? null : Path.GetDirectoryName(runDir);           // …/.pwhide
    ...
    {
        var hint = $"vault 处于管理员写保护。请手动执行：sudo pwhide --home {Hardening.Q(homeDir ?? "")} _install-staged {Hardening.Q(stagingPath)} {Hardening.Q(finalPath)}";
        if (!Hardening.Unix)
            throw new NeedsElevationException(stagingPath, finalPath, "Windows：文件 ACL 拒写，请以管理员重新运行本命令（pwhide harden 输出含 icacls 指引）");
        ...
        if (string.IsNullOrEmpty(exe))
            throw new NeedsElevationException(stagingPath, finalPath, hint + "（无法定位 pwhide 可执行文件）");

        var home = homeDir ?? Vault.DefaultHome();
        // 1) 免密 sudo（CI / 自动化），只搬运密文。子进程带 PWHIDE_CHILD_INSTALL 标记：
