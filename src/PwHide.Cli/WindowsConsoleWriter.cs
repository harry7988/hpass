...
///
/// 仅当句柄确为控制台（GetFileType == FILE_TYPE_CHAR）时启用；管道句柄由 OutputChannel 按会话
/// 控制台代码页转码处理；文件重定向/非 Windows 一律 UTF-8（重定向场景无代码页语义）。
/// </summary>
public sealed class WindowsConsoleWriter : TextWriter
{
    public const uint FileTypeChar = 0x0002;
    public const uint FileTypePipe = 0x0003;
    ...
    internal static nint StdHandle(bool stderr) => GetStdHandle(stderr ? STD_ERROR_HANDLE : STD_OUTPUT_HANDLE);
    internal static uint FileType(nint handle) => GetFileType(handle);

    internal WindowsConsoleWriter(nint handle, Stream fallback) =>
        (_handle, _fallback) = (handle, fallback);

    /// <summary>句柄是真实 Windows 控制台时创建 WriteConsoleW 写入器；否则返回 false（调用方按句柄类型选择其他通道）。</summary>
    public static bool TryCreate(Stream fallback, bool isStdHandle, bool stderr, out TextWriter writer)
    {
        writer = TextWriter.Null;
        if (!OperatingSystem.IsWindows() || !isStdHandle) return false;

        var handle = StdHandle(stderr);
        if (handle == 0 || handle == INVALID_HANDLE) return false;
        if (FileType(handle) != FileTypeChar) return false;
