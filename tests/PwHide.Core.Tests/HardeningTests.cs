...
{
    private readonly string _tmp = Path.Combine(Path.GetTempPath(), "pwhide-harden-" + Guid.NewGuid().ToString("N"));
    ...
    {
        if (!Unix) return;
        var prev = Environment.GetEnvironmentVariable("PWHIDE_NO_SUDO");
        ...
        {
            Environment.SetEnvironmentVariable("PWHIDE_NO_SUDO", prev);
