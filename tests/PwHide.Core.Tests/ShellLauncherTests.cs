...
{
    private readonly string _tmp = Path.Combine(Path.GetTempPath(), "pwhide-exec-" + Guid.NewGuid().ToString("N"));
    ...
    {
        if (!Unix) return;
        var req = Cmd(["/bin/sh", "-c", "printf %s \"$PWHIDE_TEST_V\""], shell: "none",
            env: new Dictionary<string, string> { ["PWHIDE_TEST_V"] = "env-secret-9" },
    ...
    public void ScriptMode_Pwsh_CmdletFailure_NonZeroExit()
    {
        ...
        File.WriteAllText(script, "Get-Item /pwhide-missing-xyz-9\n");
        var (exit, _) = Run(Script(script, "pwsh"));
        Assert.NotEqual(0, exit);

        File.WriteAllText(script, "nosuchcmd-pwhide-xyz --flag\n");
