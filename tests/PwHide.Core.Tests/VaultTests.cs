...
{
    public string Dir { get; } = Path.Combine(Path.GetTempPath(), "pwhide-test-" + Guid.NewGuid().ToString("N"));
...
public class VaultTests : IDisposable
{
    ...
    public void Create_MasterKeyUsesStrongIterations_AndRunDirTight()
    {
        ...
        var master = JsonSerializer.Deserialize<MasterKeyFile>(File.ReadAllText(Path.Combine(_home.Dir, "master.key")), PwHideJsonContext.Default.MasterKeyFile)!;
    ...
    public void TamperedVaultFile_DecryptFails()
    {
        ...
        var json = JsonSerializer.Deserialize<VaultFile>(File.ReadAllText(path), PwHideJsonContext.Default.VaultFile)!;
        var entry = json.Entries.Single(x => x.Name == "db");
        var ct = Convert.FromBase64String(entry.Ct);
        ct[0] ^= 0xFF;
        entry.Ct = Convert.ToBase64String(ct);
        File.WriteAllText(path, JsonSerializer.Serialize(json, PwHideJsonContext.Default.VaultFile));
    ...
    public void EntryCiphertextSwap_DetectedByAad()
    {
        ...
        // 交换两个条目的密文（模拟攻击者互换条目）
        var json = JsonSerializer.Deserialize<VaultFile>(File.ReadAllText(Path.Combine(_home.Dir, "vault.json")), PwHideJsonContext.Default.VaultFile)!;
        ...
        File.WriteAllText(Path.Combine(_home.Dir, "vault.json"),
            JsonSerializer.Serialize(json, PwHideJsonContext.Default.VaultFile));
